...
if __name__ == "__main__":
    ...
    for epoch in range(1, epoches + 1):
        # 训练一个epoch
        cifar_cnn.train_epoch(train_data, train_labels)
        
        if epoch % 5 == 0 or epoch == 1:
            train_loss, train_accuracy = cifar_cnn.val(train_data, train_labels)
            val_loss, val_accuracy = cifar_cnn.val(val_data, val_labels)
            print(f"Epoch {epoch}, 学习率: {cifar_cnn.lr}")
